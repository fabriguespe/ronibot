	...
	async run(message, args, client) {
		...
		if(args[1]=='bk'){
			...
			for(let i in users){
				...
				if((Math.round(data.hours))>24)continue
				slp+=data.in_game_slp
				message.channel.send('Cuenta #'+users[i].num+' '+data.in_game_slp+' '+Math.round(data.hours)+'hs')
			}
			...
			message.channel.send('Total '+slp+'/ $'+usd+' para que puedas reclamar.') 
