		...
		if(args[1]=='bk'){
			...
			try{
				...
				for(let i in users){
					...
					let axies=await utils.getAxiesIds(user.accountAddress.replace('ronin:','0x'))
					let metas=[]
					if(axies.axies.length==0)continue
					for(let j in axies.axies){
						let clase=axies.axies[j].class
						if(clase=='Plant')clase='Planta'
						else if(clase=='Plant')clase='Planta'
					...
					metas=metas.sort().join('-')
					if(!Object.keys(meta).includes(metas))meta[metas]=0
					else meta[metas]++
				...
				let msg=''
				for(let i in meta){
					msg+=Object.keys(meta)[i]+' '+meta[i]+'\n'
				}

				let embed = new MessageEmbed().setTitle('Metas').setDescription(msg).setColor('GREEN').setTimestamp()
				message.channel.send({content: ` `,embeds: [embed]})
				embed = new MessageEmbed().setColor('#0099ff')
