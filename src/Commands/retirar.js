...
module.exports = new Command({
	name: "retirar"+(process.env.LOGNAME=='fabrizioguespe'?'t':''),
	async run(message, args, client) {
		...
		if(args.length==2){	
			...
			await db.collection("users").updateOne({ accountAddress:quien.accountAddress},{ $set: {'nota':'retiro'}} )
			message.channel.send('El jugador fue retirado con exito')
