	...
	async run(message, args, client) {
		...
		try{
			...
			for(let ii in users){
				...
				let value='#'+user.num+"[***"+user.name+"***](https://marketplace.axieinfinity.com/profile/"+user.accountAddress+") "+user.slp_prom+(user.mmr==undefined?'':'('+user.mmr+')')+(args[1]=='all'?user.nota:'')+'('+user.puesto+')'+'\n'
